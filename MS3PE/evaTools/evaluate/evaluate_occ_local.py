...
def eval_dir(result_dir, dataset='PIOD', eval_occ=1, matlab_eng=None, maxdist=0.0075, works_num=0):
    if matlab_eng is None:
        eng = get_matlab_eng(works_num)
    else:
        eng = matlab_eng
    ...
    if dataset == 'bsds500':
        ...
        eval_res = eng.Evaluatel(result_dir, dataset, eval_occ, maxdist)
...
def eval_zip(tar_file, dataset='PIOD', eval_occ=1, matlab_eng=None, maxdist=0.0075, works_num=0):
    ...
    edge_best_t, edge_ods, edge_ois, edge_ap, ori_ods, ori_ois, ori_ap, occ_ods, occ_ois, occ_ap = eval_dir(
        result_dir, dataset, eval_occ, matlab_eng, maxdist, works_num)
...
def test_main(args):
    ...
    zip_files = get_file_name_by_suffix(
        zip_dir, '_result.zip') + get_file_name_by_suffix(zip_dir, '_result.tar')
    # print(zip_files)
    zip_file_list = list()
    if args.epochs is not None and args.epochs != '':
        epoch_info = args.epochs.split(":")
        eval_step = 1 if len(epoch_info) == 2 else int(epoch_info[-1])
        eval_epoch_list = range(int(epoch_info[0]), int(epoch_info[1])+1, eval_step)
        for i in eval_epoch_list:
            c_eval_e = f"epoch_{i}_test_result.tar"
            if c_eval_e in zip_files:
                zip_file_list.append(c_eval_e)
            else:
                print(f"epoch {i} result tar not exist!")
    # print(zip_file_list)
    # print(names33)
    ...
    log_f = None
    format_str = '' + '{:<6}	' * 10
    format_str1 = '{:.4f}	' * 10 + '{}	{}_maxdist0.0075'
    format_str2= '' + '{:<6}    ' * 4 
    format_str3 = '{:.4f}       ' * 4  + '{}    {}_maxdist0.0075'

    eng = None

    print(format_str.format('best_t', 'B-ODS', 'B-OIS', 'B-AP', 'O-ODS', 'O-OIS', 'O-AP', 'occODS', 'occOIS', 'occ_AP'))
    for file_name in zip_file_list:
        ...
        print('-' * 80)
        print('\nepochs eval list ---> \t', [get_epoch(f_name) for f_name in zip_file_list])
        ...
        print()
        print('@' * 22 + ' ' +  args.dataset+' '+file_name + ' ' + args.dataset +' '+'@' *22)  
        print('\t best_t \t', ' edgODS \t', ' edgOIS \t', ' edgeAP \t')    
        print('\t {:.4f} \t {:.4f} \t {:.4f} \t {:.4f} \t'.format(edge_best_t, edge_ods, edge_ois, edge_ap))
        print('@' * 99 + '\n')
        log_f.write('\n')
        log_f.flush()
...
def parse_args():
    ...
    parser.add_argument('--works_num', type=int, default=10, help='works_num')
...
if __name__ == '__main__':
    ...
    if args.result_dir is not None:
        ...
        start_time = time.time()
        edge_best_t, edge_ods, edge_ois, edge_ap, ori_ods, ori_ois, ori_ap, occ_ods, occ_ois, occ_ap = eval_dir(
            args.result_dir, dataset=args.dataset, eval_occ=args.occ, maxdist=args.maxdist, works_num=args.works_num)
        ...
        edge_best_t, edge_ods, edge_ois, edge_ap, ori_ods, ori_ois, ori_ap, occ_ods, occ_ois, occ_ap = eval_zip(
            args.zipfile, dataset=args.dataset, eval_occ=args.occ, matlab_eng=None, maxdist=args.maxdist, works_num=args.works_num)
        print(edge_best_t, edge_ods, edge_ois, edge_ap, ori_ods,
              ori_ois, ori_ap, occ_ods, occ_ois, occ_ap)
        print('time using: %f' % (time.time() - start_time))
        print(time.strftime('%Y-%m-%d %H:%M', time.localtime(time.time()))) 

...
class Checkpoint(object):
    ...
    def save_checkpoint(self, runner):
        ...
        state = dict(state_dict=runner.model.state_dict(),
                     ...
                     lr_scheduler_state_dict=runner.scheduler.state_dict(),
                     meta=meta,
                     cfg=runner.cfg)
        save_path = os.path.join(self.model_dir, f'lastest_cp.pth')
        torch.save(state, save_path)

    def save_state_dict(self, runner):
        save_path = os.path.join(self.model_dir, f'{runner.epoch}_st.pth')
    ...
    def save_test_results(self, runner, epoch_end=False):
        ...
        if epoch_end:
            ...
            imageio.imwrite(os.path.join(save_path, f'{image_name}_ob.png'), boundary_result)
            # imageio.imwrite(os.path.join(save_path, f'{image_name}_oo.png'), orientation_result)

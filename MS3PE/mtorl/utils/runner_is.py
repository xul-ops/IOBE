            ...
            for mode in workflow:
                ...
                if self.epoch + 1 > self.startIterEpoch:
                    if not self.training_as_is:
                        print("Current epoch traning mode: give all fn fp pairs at one time!")
                    else:
                        # true IS
                        print("Current epoch traning mode: Iteractively give one biggest fn or fp pair and train as RITM")
                        print("Iteration round in training 0 to 3")
                        if workflow != ['train']:
                            print("Iteration round in testing is ", self.max_num_iter)
                        print()
            ...
            with torch.no_grad(), self.amp_context():
                ...
                for i_iter in range(loop_iter_num):
                    ...
                    fp_masks, fn_masks, obnmsts, pos_nums, neg_nums, neg_nums2 = get_batch_masks(boundary_result, labels[:,0].cpu().numpy(), p_threshold=self.p_threshold, 
                                                                                                ...
                                                                                                epmap=edge_nms, epgraph_t=0.1, train_as_is=self.training_as_is)

...
class CanvasImage:
    ...
    def __init__(self, canvas_frame, canvas, image_name, image_shape, output_dir):
        """ Initialize the ImageFrame """
        self.image_name = image_name
        self.image_shape = image_shape
        # self.user_channel = numpy.zeros(self.image_shape)
        # print('self.user_channel.shape', self.user_channel.shape)
        # # save_path = self.image_name.split('.')[0] + '_pos.png'
        # self.user_channel[10, 10] = 255
        # cv2.imwrite('D:/Desktop/test_img.png', self.user_channel)
        self.user_click = []  # list [(), (), (), ...]
        ...
        self.coordinate_x = tk.IntVar(value=0) 
        self.coordinate_y = tk.IntVar(value=0)
        self.canvas.bind('<Button-1>', self.__left_mouse_button)  # remember canvas position
        self.canvas.bind('<B1-Motion>', self.__left_mouse_button_move)
        #self.canvas.bind('<ButtonRelease-1>', self.__left_mouse_button_released_2)
        #self.canvas.bind('<ButtonRelease-3>', self.__right_mouse_button_released)
        ...
        self.canvas.bind('<Button-3>', self.__right_mouse_button)
        ...
        self.pos_save_path = output_dir + "pos/"
        self.neg_save_path = output_dir + "neg/"
        self.pre_save_path = output_dir + "pre/"
        self.interactive_res_path = output_dir + "f_res/"
    ...
    def reload_image(self, image, reset_canvas=True):
        ...
        if reset_canvas:
            ...
            # self.image_name = image_name
            self.image_shape = [self.imheight, self.imwidth]
    ...
    def __left_mouse_button(self, event):
        self.coordinate_x.set(event.x)  # 更新画布坐标系的点的坐标
        self.coordinate_y.set(event.y)
        if self._click_callback is None:
            return
        coords = self._get_click_coordinates(event)  # 画布坐标系——》图像坐标系
        if coords is not None:
            self._click_callback(is_positive=True, x=coords[0], y=coords[1])
            # self.user_channel[coords[1], coords[0]] = 255
            # 【莫名其妙的bug】 这里引用的self.suer_channel成了不变量无法赋值，因此采用记录坐标，后续统一修改的方式
            self.user_click.append((coords[1], coords[0]))

    def __left_mouse_button_move(self, event):
        self.canvas.create_line(self.coordinate_x.get(), self.coordinate_y.get(), event.x, event.y, fill='#00ff04')
        self.coordinate_x.set(event.x)
        self.coordinate_y.set(event.y)
        coords = self._get_click_coordinates(event)
        # self.user_channel[coords[1], coords[0]] = 255
        self.user_click.append((coords[1], coords[0]))
        # print('在图像坐标系上，曲线上点的坐标为:({}, {})'.format(coords[0], coords[1]))

    def __left_mouse_button_released(self, *_args):
        # print(self.user_click)
        pos = numpy.zeros(self.image_shape)
        for item in self.user_click:
            pos[item[0], item[1]] = 1  # 255
        ...
        if previous_pos is None:
            previous_pos = np.zeros(self.image_shape)
        previous_pos = np.array(previous_pos / 255).astype(int)
        pos = np.array(pos).astype(int)
        pos = pos | previous_pos
        cv2.imwrite(save_path, pos*255)
        self.user_click = []


    def __right_mouse_button_released(self, *_args):
        pos = numpy.zeros(self.image_shape)
        for item in self.user_click:
            pos[item[0], item[1]] = 255
    ...
    def __left_mouse_button_released_2(self, *_args):
        # pos = numpy.zeros(self.image_shape)
        # for item in self.user_click:
        #     pos[item[0], item[1]] = 1  # 255
        pos = generate_interaction_maps([self.user_click], self.image_shape,
                                        interaction='point', encoding='disk',
                                        radius=3, gausigma=0.04, random_range=(-2, 2), op_way='middle', num_points=1)
    ...
    def __right_mouse_button(self, event):
        self.coordinate_x.set(event.x)  # 更新画布坐标系的点的坐标
        self.coordinate_y.set(event.y)
        if self._click_callback is None:
            return
    ...
    def __right_mouse_button_motion(self, event):
        ...
        self.canvas.create_line(self.coordinate_x.get(), self.coordinate_y.get(), event.x, event.y, fill='#ff0000')
        self.coordinate_x.set(event.x)
        self.coordinate_y.set(event.y)
        coords = self._get_click_coordinates(event)
        # self.user_channel[coords[1], coords[0]] = 255

...
if configs.use_down_sampling:
    ...
    print() 
    print("Did not use oversampling during OB generation.")
...
def projection_skiimage(model_pose_infos, camera_matrix, model_dir, img_file, save_file):
        ...
        for item in mesh_surfaces:
            ...
            if calculate_polygon_area(current_surface, point_c_3d):
                ...
                if len(fill_row_coords) == 0 or len(fill_col_coords) == 0:
                    ...
                    triangles_visible_buffer[polygons_tuple] = True
                    continue       
                ...
                for point_index in range(len(fill_col_coords)):
                    ...
                    points_info_dict, final_covered_points, triangles_visible_buffer = update_depth_dict_v5((current_h, current_w), c_point_depth,
                                                                                                           points_info_dict, f_covered_points,
                                                                                                           current_surface, point_c_3d, obj_count, 
                                                                                                           triangles_visible_buffer)
    ...
    return projected_points_info_dict, no_projection_mesh, mesh_3d_dict, vertex_map_dict, triangles_visible_buffer
...
def run_projection(model_pose_info_file, folder, projection_name):
    ...
    points_info_dict, no_projection_mesh, mesh_3d_dict, vertex_map_dict, triangles_visible_buffer = projection_skiimage(model_pose_infos, ca_m, model_dir, img_file, save_file)

    return points_info_dict, no_projection_mesh, mesh_3d_dict, vertex_map_dict, triangles_visible_buffer


def occulsion_boundary_definition(img_original, img_projected, points_info_dict, graph_projection,
                                  mesh_3d_dict, num_neighbor=4, vertex_map_dict=None, triangles_visible_buffer=None):
    same_count = 0
    length_list = list()
    ...
    for point in points_info_dict.keys():
        ...
        for item in check_neighbors:
            ...
            if graph_check_connection_new_v3(point, item, points_info_dict, graph_projection, length_list, mesh_3d_dict, vertex_map_dict, triangles_visible_buffer):
                continue
            else:
                if points_info_dict[item][0][0] < check_point_info[0]:
                    # orientation_label[point].append((item, point))
...
def run_definition(points_info_dict, folder, projection_name, graph_projection, mesh_3d_dict, vertex_map_dict, triangles_visible_buffer):
    ...
    same_count, labels_info_dict, orientation_label = occulsion_boundary_definition(img_original, img_projected,
                                                                                    ...
                                                                                    mesh_3d_dict, 4, vertex_map_dict, triangles_visible_buffer)
...
def filter_label(orientation_label, labels_info_dict, save_dir):
    ...
    # print(save_dir)
    ob_save_name = "dis_OB.png"  # "occlusion_boundary.png"
    oo_save_name = "dis_OO.png"  # "occlusion_orientation.png"
    ob3_save_name = "dis_OB3.png"
...
def run_one(model_pose_info_file, train_db=True):
    ...
    start = time.time()
    points_info_dict, no_projection_mesh, mesh_3d_dict, vertex_map_dict, triangles_visible_buffer = run_projection(model_pose_info_file,
    ...
    same_count, labels_info_dict, orientation_label = run_definition(points_info_dict, folder, projection_name,
                                                                     graph_projection, mesh_3d_dict, vertex_map_dict, triangles_visible_buffer)
...
if __name__ == '__main__':
    ...
    start = time.time()
    print("OB map original generation resolution: {}".format((configs.img_resolution_y, configs.img_resolution_x)))
    ...
    for i in range(key_min, key_max):
        ...
        # if configs.extra_rotation:
        #     c_path = configs.image_save_dir + model_pose_name + "/" + configs.extra_rot_name + "_blender_rgba.png"
        # else:
        #     c_path = configs.image_save_dir + model_pose_name + "/blender_rgba.png"
    ...
    # print(error_list)
    print("Time cost (secs) in total: ", (time.time() - start) / 60)
    print()
